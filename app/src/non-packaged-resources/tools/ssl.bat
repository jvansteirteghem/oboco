...
keytool -keystore server-ca.jks -storepass "%PASSWORD%" -certreq -alias ca | keytool -keystore server-root.jks -storepass "%PASSWORD%" -gencert -alias root -ext "bc=0" -ext "san=dns:ca" -validity 10000 -rfc > server-ca.pem
